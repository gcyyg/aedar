...
const EM_TOKEN = '***';
async function emSearch(symbol) {
    try {
        const res = await axios.get(EM_SEARCH_API, {
            params: { input: symbol, type: 14, token: EM_TOKEN, count: 1 },
            headers: { 'User-Agent': 'Mozilla/5.0', 'Referer': 'https://www.eastmoney.com' }
        });
        const hits = res.data?.QuotationCodeTable?.Data || [];
        const h = hits.find((h) => h.Classify === 'AStock' && ['1', '2'].includes(h.SecurityType));
        if (h)
            return { symbol: h.UnifiedCode || h.Code, name: h.Name || h.SecurityName, market: h.SecurityType === '1' ? '沪A' : '深A' };
    }
    catch { }
    return null;
}
