...
export function getPinyinFull(name) {
    try {
 ...
 * 根据股票代码或拼音查询本地基本信息
 */
export function getChinaStockBasic(symbol) {
    const input = symbol.replace(/\.(SH|SZ|BJ)/i, '').toUpperCase();
    // 1. 精确6位代码
    if (/^\d{6}$/.test(input)) {
        const info = CHINA_STOCKS_MAP[input];
        if (info)
            return { symbol: input, name: info.name, market: info.market, industry: info.industry, area: info.market };
    }
    // 2. 拼音首字母或全拼查找
