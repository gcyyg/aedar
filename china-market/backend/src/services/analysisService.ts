...
const MINIMAX_API_KEY = process.env.MINIMAX_DOMESTIC_API_KEY || process.env.MINIMAX_API_KEY
...
// ── Types ──────────────────────────────────────────
export type AnalysisType = 'comps' | 'dcf' | 'lbo' | 'competitive' | 'audit'
...
// ── Prompt 构建 ──────────────────────────────────────────
function buildPrompt(type: AnalysisType, data: {
  symbol: string
  name: string
  price: { current: number; pe: number; pb: number; marketCap: number; ps?: number; peg?: number; roe?: number }
  financials: { revenue: number; revenueGrowth: number; profitGrowth: number; grossMargin: number; ebitdaMargin: number; netMargin: number } | null | undefined
  peers: { symbol: string; name: string; pe: number; pb: number; marketCap: number }[] | null | undefined
  industry: string
  country: 'us' | 'china'
}): string {
...
- 市盈率(PE): ${price.pe || 'N/A'}
- 市净率(PB): ${price.pb || 'N/A'}
- 市值: ¥${price.marketCap >= 1e8 ? (price.marketCap / 1e8).toFixed(2) + '亿' : (price.marketCap / 1e4).toFixed(2) + '万'}
- 行业: ${industry}

**财务数据：**
${financials ? `- 营收增长: ${financials.revenueGrowth?.toFixed(1) || 'N/A'}%
...
${peers && peers.length > 0 ? `**可比公司（同业）：**\n${peers.map(p => `- ${p.name}(${p.symbol}): PE=${p.pe || 'N/A'}, PB=${p.pb || 'N/A'}, 市值¥${(p.marketCap / 1e8).toFixed(1)}亿`).join('\n')}` : ''}
...
- 市盈率(PE): ${price.pe || 'N/A'}
- 市净率(PB): ${price.pb || 'N/A'}
- 市值: ¥${(price.marketCap / 1e8).toFixed(2)}亿
...
${peers && peers.length > 0 ? `**主要竞争对手：**\n${peers.map(p => `- ${p.name}(${p.symbol}): PE=${p.pe || 'N/A'}, 市值¥${(p.marketCap / 1e8).toFixed(1)}亿`).join('\n')}` : ''}
...
export async function runAnalysis(
  ...
  symbol: string,
  name: string,
  priceData: { current: number; pe: number; pb: number; marketCap: number; ps?: number; peg?: number; roe?: number },
  financials: { revenue: number; revenueGrowth: number; profitGrowth: number; grossMargin: number; ebitdaMargin: number; netMargin: number } | null | undefined,
  peers: { symbol: string; name: string; pe: number; pb: number; marketCap: number }[] | null | undefined,
  industry: string,
  country: 'us' | 'china'
