...
export function getStockMarket(symbol: string, forced?: 'us' | 'china'): 'china' | 'hk' | 'us' {
  ...
  if (/^(SH|SZ|BJ)\./i.test(symbol)) return 'china'
  if (/^(sh|sz|688|002|003|300|600|601|603|000|001|002003)/i.test(s)) return 'china'
  if (/^(HK|港|\d{5})$/i.test(s)) return 'hk'
  // 拼音缩写或全拼也在A股本地库中 → 判定为A股
  if (getChinaStockBasic(s)) return 'china'
  return 'us'
}
...
function toTsCode(symbol: string): string {
  ...
  if (/^300\d{3}$/.test(s)) return `${s}.SZ`  // 创业板(3开头)
  return `${s}.SH` // 默认上交所
}
...
async function fetchPeerStockList(symbol: string, finnhubKey: string): Promise<Array<{
  ...
  try {
    ...
    type MetricResult = PromiseFulfilledResult<{ symbol: string; metric: any }>
    const validMetrics = (metricsResults as MetricResult[])
      .filter(r => r.status === 'fulfilled')
      .map(r => r.value)
...
async function fetchChinaPrice(symbol: string): Promise<StockData['price'] | null> {
  try {
    ...
    if (dailyRes.data.code !== 0 || !dailyRes.data.data.items?.length) {
      ...
      if (emPrice) return { ...emPrice, history: [] }
      return null
    }
...
async function fetchEMPrice(symbol: string): Promise<StockData['price'] | null> {
...
async function fetchChinaFundamentals(symbol: string): Promise<{pe:number, pb:number, ps:number, peg:number, marketCap:number, roe:number, revenueGrowth:number, profitGrowth:number}> {
...
async function fetchEMFundamentals(symbol: string): Promise<{pe:number, pb:number, ps:number, peg:number, marketCap:number, roe:number, revenueGrowth:number, profitGrowth:number}> {
  const code = symbol.replace(/\D/g, '').slice(0, 6)
  const isShanghai = /^(600|601|603|688)/.test(code)
  const secid = isShanghai ? `1.${code}` : `0.${code}`
  ...
  const annualData = finData.find((d: any) => d.REPORT_DATE?.includes('12-31')) || finData[0] || {}
...
// ── 东方财富 K 线兜底 ─────────────────────────────────────────────
async function fetchEMKLine(symbol: string): Promise<StockData['kline'] | null> {
  const code = symbol.replace(/\D/g, '').slice(0, 6)
  const isShanghai = /^(600|601|603|688)/.test(code)
  const secid = isShanghai ? `1.${code}` : `0.${code}`
  try {
  ...
  if (peerData.length === 0) {
    ...
    if (basicInfo) {
      peerData = Object.entries(CHINA_STOCKS_MAP)
        .filter(([code, info]: [string, any]) =>
          info.industry === basicInfo.industry && code !== symbol.replace(/\D/g, '').slice(0, 6)
        )
    ...
    if (peerData.length > 0) {
      ...
      if (quoteRes.data?.data?.diff) {
        for (let i = 0; i < peerData.length; i++) {
          ...
          if (item?.f57 > 0) peerData[i].pe = Math.round(item.f57 * 10) / 10
        }
      }
    }
  } else {
