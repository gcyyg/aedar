...
export function getStockMarket(symbol: string, forced?: 'us' | 'china'): 'china' | 'hk' | 'us' {
  ...
  if (/^(sh|sz|688|002|003|600|601|603|000|001|002003)/i.test(s)) return 'china'
...
export async function getStockData(symbol: string, forcedMarket?: 'us' | 'china'): Promise<ScoreResult | null> {
  ...
  if (peerData.length > 0) {
    ...
    if (basicInfo) {
      const peerList: typeof scoreResult.peerBenchmarks = []
      ...
      if (peerList.length > 0) {
        ...
        if (quoteRes.data?.data?.diff) {
          for (let i = 0; i < peerList.length; i++) {
            const item = quoteRes.data.data.diff[i]
            if (item?.f57 > 0) peerList[i].pe = Math.round(item.f57 * 10) / 10
          }
        }
      }
