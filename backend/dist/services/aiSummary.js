 ...
 * 生成股票 AI 分析摘要
 */
export async function generateStockSummary(symbol, name, score, cedarLevel, trackScore, growthScore, valuationScore, riskScore, marketTemp, price, industry, maEvaluation) {
    if (!MINIMAX_API_KEY) {
...
function buildSummaryPrompt(params) {
    const { symbol, name, score, cedarLevel, trackScore, growthScore, valuationScore, riskScore, marketTemp, price, industry, maEvaluation } = params;
...
function parseAndCleanSummary(raw) {
    // 移除可能的 markdown 格式
...
function formatMarketCap(cap) {
    ...
    if (cap >= 1e9)
        return `$${(cap / 1e9).toFixed(2)}十亿`;
    if (cap >= 1e6)
        return `$${(cap / 1e6).toFixed(2)}百万`;
    return `$${cap.toLocaleString()}`;
}
function generateFallbackSummary(symbol, name, score, cedarLevel, price) {
    const levelDesc = {
        S: '极佳',
        A: '优秀',
        B: '良好',
