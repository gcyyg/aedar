  ...
  app.get<{ Params: { symbol: string } }>('/:symbol', async (req, reply) => {
    ...
    try {
      ...
      const nodesData = await Promise.allSettled(
        allSymbols.map(sym => fetchCompanyInfo(sym))
      )
...
// 获取单个公司基础信息
async function fetchCompanyInfo(symbol: string): Promise<ChainNode | null> {
  try {
...
// 构建关系边
function buildEdges(nodes: ChainNode[], targetSymbol: string): ChainEdge[] {
...
// 简化评分（基于市场数据）
function calculateSimpleScore(node: ChainNode): number {
  // 简单评分逻辑
...
// 获取节点等级
function getNodeLevel(node: ChainNode): 'S' | 'A' | 'B' | 'C' {
  if (node.symbol === node.symbol) return 'S'
  if (node.industry === 'Semiconductors') return 'A'
  return 'B'
}
