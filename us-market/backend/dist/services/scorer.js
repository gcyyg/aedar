...
export function calculateScores(data) {
    ...
    // 3. 估值评分
    const valuationResult = calculateValuationScore(price, (kline?.data ?? []));
    ...
    return {
        ...
        price: {
            ...
            analyst: price.analyst ?? undefined,
            revenueGrowthCagr: price.revenueGrowthCagr ?? 0,
            profitGrowthCagr: price.profitGrowthCagr ?? 0,
            benchmarks: price.benchmarks ?? undefined,
        },
        kline: kline,
...
function calculateValuationScore(price, history) {
    ...
    const ytdData = history.filter((d) => d.date <= cutoff);
    ...
    // ── 组装 detail ────────────────────────────────────
    const percentileLabel = pricePercentile <= 30 ? '偏低' :
...
function calculateMAEval(kline, price, metric52w) {
    const result = {
        ma30: 'neutral',
        ma60: 'neutral',
        ma120: 'neutral',
